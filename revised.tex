...
The phase problem has a profound practical impact on real world experiments. 
For example, phase imaging is essential in biological studies for visualization of proteins ~\cite{Berk_2024,Yu_2025,Spoth_2017}. 
For materials samples, phase imaging allows for the visualization of low atomic number features including lithium and oxygen species in functional quantum and energy materials ~\cite{lozano2018low,kp2025electron} and probing of very electron beam sensitive hybrid structures, such as metal organic frameworks~\cite{Ma_2025,shen2020imaging}.
Effectively applying diffraction imaging techniques to real-world problems, requires an understanding of these approaches facilitating the collection of high-quality data and robust reconstruction.
Starting from mathematical formalism, we provide a comprehensive overview of the strength and limitations of various approaches.
...
\Cref{fig:pci} illustrates these effects for a simulated three-dimensional arrangement of single- and double-walled carbon nanotubes (CNTs) at low dose conditions.
The simulated projected potential is shown in \cref{fig:pci}a.
The in-focus HRTEM image (\cref{fig:pci}b) shows almost no contrast, especially for the top nanotube, which is exactly at the focal plane of the microscope.
This is a manifestation of the phase problem.
Defocus (\cref{fig:pci}c) improves visibility but introduces frequency-dependent contrast reversals, causing different regions of the CNTs to appear in or out of focus. 
Zernike phase contrast (\cref{fig:pci}d) recovers both the high-resolution lattice information and the low-frequency envelope distinguishing the CNTs from vacuum.
...
The value of upsampling is highlighted comparing the left and right columns of \cref{fig:uspsample} showing the improved resolution from computational processing. 
...
This is clear for example in \cref{fig:ctf,fig:exp-compare}a, where all the phase imaging techniques perform similarly, due to the high sampling and in-focus nature fo the data. 
...
The value of parallax or direct ptychography approaches is exemplified by the defocused dataset in cref{fig:ctf,fig:exp-compare}b, where only in the last two columns it is possible to recover atomic information.
Note the similarity in reconstruction quality for parallax imaging and single sideband. 
...
After normalizing by this variance, the resulting noise becomes unit-variance and independent of probe position, yielding the same linear SNR scaling with electron dose derived for Poisson statistics in \cite{Seki_2018,Ooe_2021} and used in the WPOA noise model \cref{eq:noise-model}.
...
In each case, iterative ptychography yields improved contrast, reduced artifacts, and more faithful recovery of structural features as compared to the reconstructions of datasets in \cref{fig:exp-compare,fig:uspsample}.
This is particular emphasized by the image Fourier transforms which show much higher spatial resolution of the reconstruction.
