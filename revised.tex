...
\usepackage{caption}
\captionsetup[figure]{name={Figure}, font=footnotesize, labelfont=bf}
...
This approach routinely provides interpretable images of materials down to atomic resolution and forms the basis of modern crystallographic characterization~\cite{Ophus_2023}.

Both TEM and STEM modalities are fundamentally limited by the microscopy ``phase problem,'' first recognized in quantum scattering by Max Born~\cite{born1926quantenmechanik} and discussed in his Nobel Lecture~\cite{born1954}.
...
The phase problem also has direct practical consequences for real-world experiments.
In biological imaging, phase contrast is essential for visualizing proteins~\cite{Berk_2024,Yu_2025,Spoth_2017}.
In materials science, it enables the detection of low atomic number species, such as lithium and oxygen in functional and quantum materials~\cite{lozano2018low,kp2025electron}, and the characterization of beam-sensitive systems, including metal–organic frameworks~\cite{shen2020imaging, Li_2025, Ma_2025}.
These challenges arise across a wide range of specimen classes, including nanoparticles~\cite{shi2025electron,Ribet_2024,Varnavides_2023}, one-dimensional nanostructures such as nanotubes~\cite{yang2016simultaneous,pelz2023solving}, two-dimensional materials~\cite{jiang2018electron,o2022increasing,Susi_2025,byrne2025fabrication,zhang2025atom}, thin films~\cite{dong2025sub}, and cross-sectional views of thicker crystalline samples~\cite{scheid2025atomic,chen2024imaging}.
...
\begin{figure*}[t]
  ...
  (a)~of single- and double-walled carbon nanotubes, acquired with an electron dose of $500~\mathrm{e}/\text{\AA}^2$ using:
  (b)~in-focus optics,
...
\Cref{fig:pci} illustrates these effects for a simulated three-dimensional arrangement of single- and double-walled carbon nanotubes (CNTs) under low-dose conditions.
The projected potential is shown in \cref{fig:pci}a.
The in-focus HRTEM image (\cref{fig:pci}b) exhibits minimal contrast, particularly for the nanotube located at the focal plane, directly reflecting the phase problem.
Introducing defocus (\cref{fig:pci}c) enhances visibility but induces frequency-dependent contrast reversals, causing different regions of the CNTs to appear alternately in and out of focus.
In contrast, Zernike phase contrast (\cref{fig:pci}d) restores both high-resolution lattice detail and the low-frequency envelope, enabling clear discrimination of the CNTs from vacuum.
...
Dark-field images in particular are highly interpretable leading to their prevalence in materials science characterization~\cite{Ophus_2023}.
...
Under the WPOA in \cref{eq:wpoa}, the specimen transmission function is $t(\boldsymbol{r}) \approx 1 + i \sigma V_p(\boldsymbol{r})$, with the Fourier transform of the scattered component given by $\tilde{\phi}(\boldsymbol{k}) = \mathcal{F}_{\boldsymbol{r}\rightarrow\boldsymbol{k}}\{\sigma V_p(\boldsymbol{r})\}$.
In this regime, the diffraction intensity can be written as the self-convolution of the converged probe $\tilde{\psi}(\boldsymbol{k})$ with the Fourier transform of the transmission function $\tilde{t}(\boldsymbol{k}) = \delta(\boldsymbol{k}) + i \tilde{\phi}(\boldsymbol{k})$ term~\cite{Rodenburg_1993}:
\begin{align}
  I(\boldsymbol{R},\boldsymbol{k}) = \iint\, &\tilde{\psi}(\boldsymbol{k}')
  \tilde{t}(\boldsymbol{k}-\boldsymbol{k}')
...
For a pixelated detector, $D_j(\boldsymbol{k})=\delta(\boldsymbol{k}-\boldsymbol{k}_j)$\footnote{Note that, for notational convenience, in what follows we simply use $\boldsymbol{k}$ instead of $\boldsymbol{k}_j$ for the detector pixel coordinate.}, the integrand reduces to the aperture-overlap function $\Gamma(\boldsymbol{q},\boldsymbol{k})$.
...
In the absence of aberrations, $\chi(\boldsymbol{k})=0$, and summing over all detector pixels $D_j$ which corresponds to a detector with uniform weighting across the bright-field disk, the real part of \cref{eq:ctf-corr-real} vanishes and the imaginary part reduces to the aperture autocorrelation:
\begin{align}
...
\begin{equation}
  \mathrm{SSNR}(\boldsymbol{q}) = \frac{\lvert \langle \tilde{\psi}_{\mathrm{rec}}(\boldsymbol{q}) \rangle \rvert} {\sqrt{\operatorname{Var}\!\left[\tilde{\psi}_{\mathrm{rec}}(\boldsymbol{q})\right]}},
  \label{eq:ssnr}
\end{equation}
which quantifies the statistically reliable recoverable information at each spatial frequency $\boldsymbol{q}$ of the recostructed object, $\tilde{\psi}_{\mathrm{rec}}(\boldsymbol{q})$.
...
In the absence of aberrations, these two ``sidebands" provide complex-conjugate estimates of the same object Fourier component, consistent with Friedel’s law.\footnote{Friedel's law illustrates how a single mathematical principle -- the Hermitian conjugation symmetry of the Fourier transform of a real-valued function -- connects the physics of imaging with crystallographic structure determination~\cite{Friedel_1913,karle1985}.}
Accordingly, the specimen phase $\tilde{\phi}(\boldsymbol{q})$ can be reconstructed using only one of them.
In practice, once aberrations are present, the two overlap regions acquire different geometric phase factors arising from the probe aberration function $\chi(\boldsymbol{k})$.
...
The impact of upsampling is demonstrated in~\cref{fig:uspsample} by comparing the left and right columns, where computational upsampling leads to a clear improvement in spatial resolution and feature definition. 
...
This behavior is evident in~\cref{fig:ctf,fig:exp-compare}a, where all phase-retrieval methods yield comparable reconstructions due to the high sampling and near in-focus acquisition conditions.
The advantage of parallax and direct ptychography becomes apparent in the defocused dataset (\cref{fig:ctf,fig:exp-compare}b), where only these approaches recover atomic-scale information.
Notably, the reconstruction quality of parallax closely matches that of single-sideband (SSB), consistent with its interpretation as a quadratic approximation to the full phase-compensated inversion.

The SSB, OBF, and MF/WDD techniques compensate for the aperture-overlap transfer function using knowledge of the probe wavefunction, allowing recovery of the object phase even in the presence of residual higher-order aberrations~\cite{Varnavides_2025,Susi_2025}.
MF/WDD performs this compensation through an explicit Wiener deconvolution, while SSB and OBF incorporate the probe phase directly into their reconstruction kernels.
However, ptychographic acquisitions on aberration-corrected instruments are often dominated by low-order aberrations such as defocus and astigmatism. 
In this regime, the quadratic phase approximation underlying parallax provides an effective description of the probe phase, yielding reconstructions that are nearly identical to those obtained with SSB, OBF, or MF/WDD~\cite{Varnavides_2025_ssnr}.
...
i \operatorname{Im}\!\left[\tilde{\phi}(\boldsymbol{q}) \,\thinspace \Gamma(\boldsymbol{q},\boldsymbol{k})\right]
\nonumber\\
&+
\frac{1}{2}
...
In practice, this quadratic correction has little effect on BF direct ptychography reconstructions.
...
Phase-compensated direct ptychography operators such as SSB explicitly project onto the odd (phase) channel, so the quadratic contribution lies in the nullspace of the inverse operator.
While third-order terms are again odd and therefore non-zero, they scale as $\tilde{\phi}^3$ and do not factorize into a simple object–probe product, making them both negligible and incompatible with direct inversion, rendering BF direct ptychography effectively self-linearizing for moderate phase shifts.

While WDD and iCOM imaging are sometimes introduced in the literature using the strong phase object approximation, their closed-form inversion kernels rely on the same linearized signal model used here.
...
Although electron detection is fundamentally Poissonian, at moderate electron counts the shot noise can be approximated as Gaussian with variance proportional to the mean intensity. 
After normalization by this variance, the noise becomes unit-variance and independent of probe position, yielding the same linear SNR scaling with electron dose derived for Poisson statistics in Refs. \citenum{Seki_2018,Ooe_2021} and used in the WPOA noise model \cref{eq:noise-model}.
...
In all cases, iterative ptychography yields improved contrast, reduced artifacts, and more faithful recovery of structural features compared to the direct reconstructions shown in~\cref{fig:exp-compare,fig:uspsample}.
...
Phase-retrieval techniques in STEM provide a unified framework for recovering specimen phase and overcoming the intrinsic contrast limitations imposed by the phase problem.
...
These methods are broadly applicable to beam-sensitive and low-atomic-number materials, including biological systems~\cite{Berk_2024,Yu_2025,Spoth_2017}, polymers~\cite{bardot2025mechanically}, and hybrid structures~\cite{Li_2025,Ma_2025,shen2020imaging,ding2022three}, as well as to inorganic materials where sensitivity to light elements is critical~\cite{lozano2018low,kp2025electron}.

In practice, achieving reliable phase retrieval depends sensitively on experimental conditions.
...
Recent work has demonstrated that ptychographic phase retrieval can be performed even on uncorrected instruments~\cite{nguyen2024achieving}, including at low accelerating voltages and small convergence angles~\cite{blackburn2025sub}.
Nevertheless, most applications continue to benefit from aberration correction and advanced instrumentation, and practical workflows still rely heavily on fast pixelated detectors and carefully optimized acquisition parameters.

These considerations highlight several important limitations and open challenges.
...
At the same time, experimental parameter selection, including defocus, probe size, and scan sampling, remains a critical and nontrivial aspect of successful reconstruction, particularly under low-dose conditions where noise and incomplete sampling can lead to instability or non-uniqueness.

From a practical perspective, direct reconstruction techniques offer a complementary advantage in computational efficiency, enabling live reconstruction during data acquisition using modern hardware~\cite{Yu_2022,Ooe_2021,Pelz_2022,Strauch_2021}.
Such real-time feedback is valuable for optimizing imaging conditions, assessing sample thickness, and monitoring beam damage.
In typical workflows, these fast direct methods are used during acquisition, while more complete iterative reconstructions—especially those incorporating multislice or mixed-state models—are performed offline on GPU-equipped systems.

Looking forward, further progress will depend on addressing these limitations in a systematic way.
...
Extending phase retrieval to thicker and more strongly scattering specimens will require tighter integration of multislice forward models with efficient inversion schemes.
Finally, scaling these approaches to larger fields of view and higher-dimensional modalities—including tomography and time-resolved measurements—remains a key challenge for both algorithm development and instrumentation.
